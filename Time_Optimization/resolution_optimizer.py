...
class ResolutionPerformanceOptimizer:
    ...
    def optimize_resolution(self, video_path: str, out_name: str, teams_colors: List[str],
        ...
        if test_percentage < 100.0:
            num_to_test = max(1, int(len(all_resolutions) * test_percentage / 100.0))
            ...
            print(f"Testing {test_percentage}% of resolutions ({num_to_test}/{len(all_resolutions)}) - ")
            print(f"{'Higher' if prefer_higher_resolution else 'Lower'} resolution preference")
        ...
        if test_percentage < 100.0:
            ...
            skipped = all_resolutions[num_to_test:] if prefer_higher_resolution else all_resolutions[:-num_to_test]
            for res in skipped:
...
def run_optimization_example():
    ...
    teams_colors = ['white', 'white', 'blue', 'blue', 'black', 'yellow']  # Example colors
...
def run_quick_test_example():
    ...
    optimizer.plot_comprehensive_timing_analysis(save_path="./Out/quick_timing_analysis.png")
